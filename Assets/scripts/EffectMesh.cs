...
namespace Meta.XR.MRUtilityKit
{
    public class EffectMesh : MonoBehaviour
    {
        ...
        [Tooltip("The first material applied to the generated mesh.")]
        [FormerlySerializedAs("_MeshMaterial")]
        public Material MeshMaterial1;

        [Tooltip("The second material applied to the generated mesh.")]
        ...
        public void ToggleEffectMeshVisibility(bool shouldShow, LabelFilter label = new LabelFilter(), Material materialOverride = null)
        {
            foreach (var kv in effectMeshObjects)
            {
                ...
                if (kv.Value.effectMeshGO && filterByLabel)
                {
                    ...
                    if (materialOverride)
                    {
                        kv.Value.effectMeshGO.GetComponent<MeshRenderer>().materials = new Material[] { materialOverride, kv.Value.effectMeshGO.GetComponent<MeshRenderer>().materials[1] };
                    }
                }
            }
        }
        ...
        public void OverrideEffectMaterial(Material newMaterial, LabelFilter label = new LabelFilter())
        {
            foreach (var kv in effectMeshObjects)
            {
                ...
                if (kv.Value.effectMeshGO && filterByLabel)
                {
                    kv.Value.effectMeshGO.GetComponent<MeshRenderer>().materials = new Material[] { newMaterial, kv.Value.effectMeshGO.GetComponent<MeshRenderer>().materials[1] };
                }
            }
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        EffectMeshObject CreateEffectMesh(MRUKAnchor anchorInfo, float border)
        {
            ...
            // only attach MeshRenderer if materials have been assigned
            if (MeshMaterial1 != null && MeshMaterial2 != null)
            {
                MeshRenderer newRenderer = newGameObject.AddComponent<MeshRenderer>();
                newRenderer.materials = new Material[] { MeshMaterial1, MeshMaterial2 };
        ...
        async void CreateGlobalMeshObject(MRUKAnchor globalMeshAnchor)
        {
            ...
            if (MeshMaterial1 != null && MeshMaterial2 != null)
            {
                renderer.materials = new Material[] { MeshMaterial1, MeshMaterial2 };
            }
